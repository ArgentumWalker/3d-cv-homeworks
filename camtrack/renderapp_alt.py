...
class CameraTrackRendererApp:
    ...
    def __init__(self, cam_model_files,
        ...
        self._camera = recordclass('CameraPoseAndParameters', 'z_rot x_rot pos fov_y')(
    ...
    def animate(self):
        ...
        if self._data.key_states[ord(b'a')]:
            self.move_camera(d_time * np.linalg.inv(self.camera_rot_mat()).dot([-self._change_rates.tr_xz, 0, 0]))
        if self._data.key_states[ord(b'd')]:
            self.move_camera(d_time * np.linalg.inv(self.camera_rot_mat()).dot([+self._change_rates.tr_xz, 0, 0]))
        if self._data.key_states[ord(b's')]:
            self.move_camera(d_time * np.linalg.inv(self.camera_rot_mat()).dot([0, 0, +self._change_rates.tr_xz]))
    ...
    def mouse_move(self, pos_x, pos_y):
        ...
        self._data.last_xy = (pos_x, pos_y)
        self._camera.z_rot += change_x * self._change_rates.camera
        self._camera.z_rot %= np.pi * 2
        self._camera.x_rot += change_y * self._change_rates.camera
        self._camera.x_rot = np.clip(self._camera.x_rot, -np.pi / 2, np.pi / 2)
    ...
    def camera_rot_mat(self):
        return np.array([[1, 0, 0], [0, np.cos(self._camera.x_rot), -np.sin(self._camera.x_rot)], [0, np.sin(self._camera.x_rot), np.cos(self._camera.x_rot)]]).dot(
            [[np.cos(self._camera.z_rot), 0, np.sin(self._camera.z_rot)], [0, 1, 0], [-np.sin(self._camera.z_rot), 0, np.cos(self._camera.z_rot)]])
